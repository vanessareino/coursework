...
```{r load-counts}
year_counts <- read_tsv("./week3/ngrams/year_count.tsv", col_names = FALSE) %>%
rename(term = X1, year = X2, volume = X3, book_count = X4 )
summary(year_counts)

total_counts <- read_csv("./week3/ngrams/total_counts.csv", col_names = FALSE) %>%
rename(year = X1, total_volume = X2, page_count = X3, book_count = X4 )
...
The total number of lines in the year_counts data frame is 'nrow(year_counts)'
The total number of lines in the total_counts data frame is 'nrow(total_counts)'
# Part B
...
join_years_totals <- left_join(year_counts, total_counts, by = "year") %>%
